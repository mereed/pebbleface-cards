...
Card* card_add(Window *window, char *location_buff_ptr, char* conditions_buff_ptr, char* temperature_buff_ptr)
{
	...
	this->background_layer = bitmap_layer_create(GRect(bounds.origin.x, bounds.origin.y + bounds.size.h - 10, 144, 129));
	...
	this->conditions_layer = cl_text_layer_create(GRect(bounds.origin.x + 36, bounds.origin.y + 53, 129, 30), GColorBlack, GColorClear, false, NULL, FONT_KEY_GOTHIC_24, GTextAlignmentLeft);
	this->temperature_layer = cl_text_layer_create(GRect(bounds.origin.x + 36, bounds.origin.y + 53, 129, 30), GColorBlack, GColorClear, false, NULL, FONT_KEY_GOTHIC_24, GTextAlignmentLeft);
	this->location_layer = cl_text_layer_create(GRect(bounds.origin.x + 36, bounds.origin.y + 75, 129, 30), GColorBlack, GColorClear, false, NULL, FONT_KEY_GOTHIC_18, GTextAlignmentLeft);
	...
	{
		strftime(this->time_buff, sizeof("00:00"), "%l:%M", cl_get_time());
	}
	...
	else
	{
		...
		if(time->tm_hour > 6 && time->tm_hour < 18)
		{
			//Day
			if(
				 strstr(conditions_buff_ptr, "clear") != NULL
			|| strstr(conditions_buff_ptr, "Clear") != NULL
			)
			{
				//Show sunny
				this->weather_bitmap = gbitmap_create_with_resource(RESOURCE_ID_SUN);
				 ...
				 strstr(conditions_buff_ptr, "rain") != NULL
			|| strstr(conditions_buff_ptr, "Rain") != NULL
			)
			{
				//Show rainy
				this->weather_bitmap = gbitmap_create_with_resource(RESOURCE_ID_RAIN);
				 ...
				 strstr(conditions_buff_ptr, "cloud") != NULL
			|| strstr(conditions_buff_ptr, "Cloud") != NULL
			)
			{
				//Show rainy
				this->weather_bitmap = gbitmap_create_with_resource(RESOURCE_ID_CLOUD);
				 ...
				 strstr(conditions_buff_ptr, "wind") != NULL
			|| strstr(conditions_buff_ptr, "Wind") != NULL
			)
			{
				//Show windy
				this->weather_bitmap = gbitmap_create_with_resource(RESOURCE_ID_WIND);
				 ...
				 strstr(conditions_buff_ptr, "thunder") != NULL
			|| strstr(conditions_buff_ptr, "Thunder") != NULL
			)
			{
				//Show rainy
			...
			else
			{
				...
				app_log(APP_LOG_LEVEL_INFO, "Cards", 0, "Unknown weather type: %s", conditions_buff_ptr);
				this->weather_bitmap = gbitmap_create_with_resource(RESOURCE_ID_UNKNOWN);
			}
		}
		else
	...
	text_layer_set_overflow_mode(this->temperature_layer, GTextOverflowModeTrailingEllipsis);
	text_layer_set_text(this->temperature_layer, temperature_buff_ptr);
	layer_add_child(window_get_root_layer(window), text_layer_get_layer(this->temperature_layer));
...
void card_destroy(Card *this)
{
	...
	text_layer_destroy(this->temperature_layer);
...
static void card_animate(Card *this, GRect start, GRect finish, int duration, int delay, AnimationCurve curve)
{
	...
	animate_layer(
		...
		GRect(start.origin.x + 36, start.origin.y + 53, 129, 30),
		GRect(finish.origin.x + 36, finish.origin.y + 53, 129, 33),
		duration, delay, curve
	);

	//temperature
	animate_layer(
		text_layer_get_layer(this->temperature_layer),
		GRect(start.origin.x + 36, start.origin.y + 80, 129, 30),
		GRect(finish.origin.x + 36, finish.origin.y + 80, 129, 33),
		duration, delay, curve
	);
	...
	animate_layer(
		...
		GRect(start.origin.x + 36, start.origin.y + 97, 129, 30),
		GRect(finish.origin.x + 36, finish.origin.y + 97, 129, 33),
		duration, delay, curve
	);
}
